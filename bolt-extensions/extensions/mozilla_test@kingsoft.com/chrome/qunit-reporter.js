...
(function() {
	...
	QUnit.jUnitReport = function(report) {
		var result = 'SUCCEED';
		if (report.results.failed && report.results.failed > 0)
			result = 'FAILED';

		var finalResult = '\n' + 'Final-Result-' + result + ' | Total-Module-Runed: ' + report.results.total
							+ ' | Total-Passed: ' + report.results.passed + ' Total-Failed: ' + report.results.failed + '\n';

		var reportFile = "report.xml";
        var file = Services.dirsvc.get("TmpD", Ci.nsIFile);
        file.append(reportFile);
        console.logStringMessage(file.path);
        if (!file.exists()) {
            file.createUnique(0, parseInt(0x0666, 8));
        }

		// file is nsIFile, data is a string
		var foStream = Components.classes["@mozilla.org/network/safe-file-output-stream;1"]
        ...
        foStream.write(report.xml, report.xml.length);
        foStream.write(finalResult, finalResult.length);
	...
	QUnit.log(function(data) {
		...
		if (!data.result) {
			if (currentRun) {
				for (var m = 0; m < currentRun.modules.length; m++) {
					if (currentRun.modules[m].name == data.module){
						var parentModule = currentRun.modules[m];
						for (var n = 0; n < parentModule.tests.length; n++){
							if (parentModule.tests[n].name == data.name)
							{
								parentModule.tests[n].failedAssertions.push(data);
								parentModule.stdout.push('[' + parentModule.name + ', ' + parentModule.tests[n].name + ', ' + assertCount + '] ' + data.message);
								break;
							}
						}
					}	
			...
			for (var m = 0; m < currentRun.modules.length; m++) {
				if (currentRun.modules[m].name == data.module){
					...
					for (var n = 0; n < parentModule.tests.length; n++){
						if (parentModule.tests[n].name == data.name)
						{
							parentModule.tests[n].time = (new Date()).getTime() - currentTest.start.getTime();  // ms
							parentModule.tests[n].total = data.total;
							parentModule.tests[n].passed = data.passed;
							parentModule.tests[n].failed = data.failed;
							break;
						}
					}
				}	
	...
	QUnit.moduleDone(function(data) {
		if (currentRun) {
			for (var m = 0, mLen = currentRun.modules.length; m < mLen; m++) {
				if (currentRun.modules[m].name == data.name){
					currentRun.modules[m].time = (new Date()).getTime() - currentModule.start.getTime();  // ms
					currentRun.modules[m].total = data.total;
					currentRun.modules[m].passed = data.passed;
					currentRun.modules[m].failed = data.failed;
					break;
				}	
			}
		}
	...
	QUnit.done(function(data) {
		console.logStringMessage('Qunit Done!!!!!!!!!!');
