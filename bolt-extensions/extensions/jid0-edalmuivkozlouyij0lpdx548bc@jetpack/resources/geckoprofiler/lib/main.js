...
let ThunderbirdAppWrapper = {
  ...
  init: function tb_init() {
    ...
    this.initProfiler(Profiler, {}, function() {
      dump("The profilerIsActive:" + profilerIsActive + "\n");
      if (profilerIsActive)
      ...
      timers.setInterval(function() {
        ...
        get_profile(
          function(aMsg) {
            //console.log("Something coming:" + JSON.stringify(aMsg.progress));
          },
          function(aProfile) {
            //console.log('Completed getting profile');
            console.log("The profile length:" + aProfile.length + "\n");
            console.log("The type of log is:" + typeof(aProfile));
            Task.spawn(function*(){
              let rootPath = OS.Constants.Path.profileDir
              let profilePath = OS.Path.join(rootPath, 'profile');
              try {
                if (profileId == 0) {
                  yield OS.File.removeDir(profilePath, {ignoreAbsent: true, ignorePermissions: true});
                  yield OS.File.makeDir(profilePath, {ignoreExisting: true, from: rootPath});
                }
                yield OS.File.writeAtomic(OS.Path.join(profilePath, profileId + ".profile"), aProfile, {encoding: "utf-8"});
              } catch (error) {
                console.log(error);
              }
...
exports.main = function(aOptions, aCallbacks) {
  if (prefs.get_pref_bool("profiler.", "disabled") === true) {
    dump("Disable performance profiler\n");
    return;
  }
  dump("Start performance profiler\n");
  if (require("api-utils/xul-app").is("Thunderbird"))
    appWrapper = ThunderbirdAppWrapper;
  else
    appWrapper = FirefoxAppWrapper;

  dump("appWrapper.init():" + JSON.stringify([aOptions, aCallbacks]));
  appWrapper.init();
}
